...
Таким образом в зависимости от порядка множителей $\forall i \in [1; n -1], j \in [2; n]$ на место $(i, j)$ может прийти либо элемент бывший в $A$ в позиции $(i + 1, j)$ (снизу), либо в позиции $(i, j - 1)$ (слева).

Для $i = n$  $\forall j \in [2; n]$ на место $(i, j)$ может прийти элемент с позиции $(i, j - 1)$ или $(1, j)$.

Для $j = 1 \,\, \forall i \in [1; n - 1]$ на место $(i, j)$ может прийти элемент с позиции $(i, n)$ или $(i + 1, j)$.

Для $i = n \land j = 1$ на место $(i, j)$ может прийти элемент $(n, n)$ или $(1, 1)$.

Построим такую матрицу с левого верхнего угла (обозначим его за $a_1$ и применим ограничения выше)
$$
\begin{pmatrix}
a_1&?&?&?&\cdots&?&?&?\\
?&a_1&?&?&\cdots&?&?&?\\
...
\end{pmatrix}
$$
теперь пометим второй элемент в первом ряду за $a_2$ и применим ограничения
$$
\begin{pmatrix}
a_1&a_2&?&?&\cdots&?&?&?\\
?&a_1&a_2&?&\cdots&?&?&?\\

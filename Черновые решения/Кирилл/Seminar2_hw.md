...
## 5
Из семинара мы знаем, что с матрицей нециклических сдвигов
$$
SHIFT =
\begin{pmatrix}
0&1&0&0&\cdots&0&0\\
...
коммутирует любая матрица для которой верно, что в первом столбце нули, в последней строке нули, а так же верно что $\forall i \in [2; n], j \in [1; n - 1]: \,\, A_{i - 1, j} = A_{i, j + 1}$, то есть имеющая вид
$$
A = 
\begin{pmatrix}
a_1&a_2&a_3&\cdots&a_{n-1}&a_n\\
...
\end{pmatrix}
$$
для матрицы циклических сдвигов
$$
CSHIFT = 
\begin{pmatrix}
0&1&0&0&\cdots&0&0\\
...
\end{pmatrix}
$$
так же должно выполняться условие  $\forall i \in [1; n - 1], j \in [2; n]: \,\, A_{i + 1, j} = A_{i, j - 1}$ (в силу того что в зависимости от порядка множителей на место $(i, j)$ может прийти элемент "слева" или "снизу").

Но для последней строки и первого столбца это тоже должно выполняться (так как сдвиг циклический, при этом для первого столбца ).
